    ...
    return (
        <Container className={"pt-5 pb-5"}>
            ...
            <ul>
                ...
                <li>Interactive solving of linear optimization problems with different algorithmic variants</li>
            </ul>
            ...
            <ul>
                ...
                <li>Floating-point numbers with the delimiter <code>.</code> like: <code>1.5</code></li>
            </ul>
            <p>No other number input formats are supported; for example, <code>-0,55746</code> will trigger an error.</p>

            <h4 className={"mt-4"}>Comments</h4>
            <p>Comments in MPS inputs are supported and start with the <code>#</code> character. Everything between the first <code>#</code> character and the end of the line is ignored. For example, the line: <code>x_1        OBJ   #  1\n</code> will be processed as <code>x_1        OBJ</code>.</p>

            <h4 className={"mt-4"}>LaTeX support</h4>
            <p>Simpler uses LaTeX formatting in all non-error outputs. This means that entering a variable name like <code>x_1</code> will result in it being rendered as: <InlineMath math={"x_1"} />. Note that this applies to all nameable components of the MPS input format (for example, RHS names as well).</p>

            <p><i>Example MPS input:</i></p>
            <div
                style={{
                    width: '100%',
            ...
            <ul>
                ...
                <li>Multiplicative simplex algorithm (two-phase simplex algorithm, edited version of the revised method with basis matrix inverse calculation optimizations)</li>
            </ul>

            <h3 className={"mt-4"}>Troubleshooting</h3>
            <p>Simpler caches backend inputs and outputs in the frontend IndexedDB, which may occasionally cause minor problems. The first step in troubleshooting is clicking the "Clean storage" button at the
                bottom of your screen. If that doesn't help and the problem persists, try restarting Simpler, opening it in a different browser, or opening it in private mode. Try to keep your application updated by downloading the newest version when it comes out on <Link to={"https://github.com/jakvitov/simpler/releases"}>GitHub</Link>.</p>
            <h3 className={"mt-4"}>Bug reporting</h3>
            <p>Reporting bugs helps Simpler advance and provide a better user experience. If any error occurs while using the application, a bug report link will appear. If you click it, it will either contain prefilled
                information about an application-caught error, or ask you to describe your problem if none was caught. In the case of a prefilled bug report, you get a full overview of what is being sent, so that you can personally check for sensitive information.</p>
            <p>Try to always include your Simpler version and the input that triggered the bug, so that it can be tracked down and repaired swiftly.</p>
        </Container>
    )
}
