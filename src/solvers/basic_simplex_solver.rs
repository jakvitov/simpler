...
pub fn solve_basic_simplex(simplex_table: &mut BasicSimplexTable, html_output: &mut HtmlOutput) -> Result<Rational, Box<NumericalError>> {
    ...
    loop {
        ...
        if pessimal_column.is_none() {
            ...
            return Ok(simplex_table.objective_rhs);
        }

        // Iteration
        basic_simplex_iteration(simplex_table, &pessimal_column.unwrap(), iteration_counter,  html_output, &mut gcd_cache)?;
        iteration_counter += 1;
    }
}

/// Perform one simplex iteration with output to the HtmlOutput
fn basic_simplex_iteration(simplex_table: &mut BasicSimplexTable, pessimal_column: &(usize, Rational), n: usize,
                           html_output: &mut HtmlOutput, gcd_cache: &mut GcdCache) -> Result<(), Box<NumericalError>> {

    html_output.start_simplex_iteration(n);

    let mut t_vec = get_t_vector(simplex_table, pessimal_column, gcd_cache)?;
    // Pivot calculation
    t_vec.iter().for_each(|element| {debug_assert!(element.is_positive())});
    let pivot = get_pivot(&t_vec, pessimal_column);
...
fn get_pivot(t_vec: &Vec<Rational>, pessimal_column: &(usize, Rational)) -> (usize, usize) {
    ...
    for (index, value) in t_vec.iter().enumerate() {
        if (min_value.is_some() && min_value.unwrap() > value) || min_value.is_none() {
            min_value = Some(value);
            min_index = index;
        }
    }
    //Only case when this is none is when the t_vec is empty
...
mod tests {
    ...
    use crate::solvers::basic_simplex_solver::{basic_simplex_iteration, get_pivot, get_t_vector, solve_basic_simplex};
    ...
    #[test]
    fn check_basic_simplex_iteration_succeeds() {
        let mut simplex_table = create_minimal_simplex_table_for_testing();
        let mut html_output = HtmlOutput::with_application_header();
        let mut gcd_cache = GcdCache::init();
        let res = basic_simplex_iteration(&mut simplex_table, &(1, Rational::from_integer(-2)), 1, &mut html_output, &mut gcd_cache);
        assert!(res.is_ok());
